...
@tool
async def generate_social_post(transcript: str, image_url: str, callbacks: Callbacks) -> str:
    """Generate social post content."""
    print(f'generatePostContent')

    post_template = PromptTemplate.from_template(
    """You are an AI language model assistant. Your objective is to create SEO friendly 5 tweets from the transcript.:
    ...
    chunks = [chunk async for chunk in chain.astream({"transcript": transcript, "image_url" : image_url})]
